...
def compute_class_reas_loss(args, logits, labels, device, seq_len, loss_func, compute_acc=False, tokenizer=None, it=None):
    ...
    if args.loss_type == "sequence-level":
        print("Compute Sequence Level Loss")
        ...
        for i in range(labels.size(0)):
            ...
            print("logits raw",logits, logits.shape)
            print("labels raw",labels, labels.shape)
            # Seperate as colon, respect padding offset for labels
            class_logits = logits[i, :colon_pos, :]
            class_labels = labels[i, :colon_pos]  
            ...
            reason_logits = logits[i, colon_pos:, :] #last indexed removed to compensate for the offset padding token in class labels
            reason_labels = labels[i, colon_pos:]
            ...
            if reason_len[0][0].item() <= 3:
                ...
                print("class_loss content", class_logits.view(-1, class_logits.size(-1)), class_logits.view(-1, class_logits.size(-1)).shape, "\n",class_labels.view(-1),class_labels.view(-1).shape)
                loss = loss_func(
                ...
                class_losses.append(loss.view(-1))

            else:
                #print("Compute loss for both classification and reasoning")
                #class_loss = loss_func(class_logits, class_labels)
                print("old loss func content",reason_logits.reshape(-1, reason_logits.size(-1)),reason_logits.reshape(-1, reason_logits.size(-1)).shape, "\n",reason_labels.reshape(-1),reason_labels.reshape(-1).shape)
                #reason_loss = loss_func(reason_logits.reshape(-1, reason_logits.size(-1)), reason_labels.reshape(-1))
                #reason_loss = loss_func(reason_logits, reason_labels)

                # Reshape for NN.CrossEntropyLoss
                class_loss = loss_func(
                    class_logits.view(-1, class_logits.size(-1)), 
                    class_labels.view(-1)
                )
                reason_loss = loss_func(
                    reason_logits.view(-1, reason_logits.size(-1)), 
                ...
                #seq_losses.append(loss.view(-1))
                class_losses.append(class_loss.view(-1))
                reason_losses.append(reason_loss.view(-1))
            ...
            if it % 500 == 0 and i <= 2:
                ...
                print("class prediction",tokenizer.decode(torch.argmax(class_logits, dim=-1), skip_special_tokens=False),"\n")
                print("reason_label",tokenizer.decode(reason_labels[reason_labels != -100]), reason_labels.shape,"\n")
                print("reason prediction",tokenizer.decode(torch.argmax(reason_logits, dim=-1)),"\n")
        ...
        if args.warmup_iters:
            ...
            if it >= args.warmup_iters:
                ...
                # Calculate the linear interpolation between start_value and target_value to schedule loss ratio
                scale = min(1.0, it / args.warmup_iters)
                loss_ratio = 0 + scale * (args.loss_ratio - 0)
        ...
        batch_loss_class = torch.mean(torch.cat(class_losses, dim=0))
        #print("batch_loss_class", batch_loss_class, batch_loss_class.shape)
        if reason_losses:
            batch_loss_reason = torch.mean(torch.cat(reason_losses, dim=0))
            print("batch_loss_reason", batch_loss_reason, batch_loss_reason.shape)
            batch_loss = loss_ratio * batch_loss_class + (1 - loss_ratio) * batch_loss_reason#(batch_loss_reason / batch_loss_reason.detach())


        else:
            batch_loss_reason = torch.zeros(1, device=device)
            batch_loss = batch_loss_class
    ...
    # return loss, discard_loss, batch_loss_class, batch_loss_reason, token_level_acc, tokenizer.decode(first_token_labels)
...
def finetune(args, tokenizer: T5Tokenizer, model: deepspeed.DeepSpeedEngine, optimizer: Adafactor, dataset, device, teacher_model=None):
    ...
    if args.refinement:
        ...
        # few shot examples
        # lower reason ratio in beginning
        # Weigh reason labels for true mean
        # Langaue mohel loss du hunshchwzgsicht
        # nan in epoch 0 it 0
        #first word cu toff in l012
        og_labels =[]
            # Load each line
    ...
    for epoch in range(args.epochs):
        ...
        for it, (model_batch, no_model_batch, gen_data) in enumerate(train_dataloader):# Check for NaN/Inf in input data
            ...
            if args.refinement:
                ...
                # torch.set_printoptions(profile="full")
                # print("uids", no_model_batch["uids"])
                # print("premises", no_model_batch["premises"])
                # print("hypotheses", no_model_batch["hypotheses"])
                # print("labels", no_model_batch["labels"])
                # torch.set_printoptions(profile="default")
                ...
                # print("decoded_outputs", decoded_outputs, len(decoded_outputs))
                # print("input_ids", decoded_inputs, len(decoded_inputs))
                # print("labels", decoded_labels, len(decoded_labels))
                # print("uids", decoded_uids, len(decoded_uids))
                # print("premises", decoded_premises, len(decoded_premises))
                # print("hypotheses", decoded_hypotheses, len(decoded_hypotheses))

                # for i, item in enumerate(decoded_outputs):
                #     print(f"Output {i}: '{item}', Length: {len(item)}, Split result: {item.split()}")
                #     print("labels", no_model_batch["labels"][i])
                #     print(gen_model_inputs["input_ids"][i])
            ...
            if it % 300 == 0:
                ...
                #acc_outputs = [item.lower().split(':', 1)[0] for item in decoded_outputs]
    ...
    with torch.no_grad():
        for it, (model_batch, no_model_batch, gen_data) in enumerate(tqdm(dataloader, desc="Evaluating", disable=(dist.get_rank() != 0))):
            ...
            if it % 5 == 0:
                ...
                # First words (classifications) of batch
                acc_labels =[item.split(':')[0].lower().rstrip(':') for item in decoded_labels]
                #acc_outputs = [s[0].lower().split(':', 1)[0] for s in decoded_outputs]
                try:
                    acc_outputs = [item.lower().split(':', 1)[0].split()[0] for item in decoded_outputs]
