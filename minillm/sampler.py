...
class PPOSampler():
    ...
    def __init__(
        ...
        print("###max_length in train_minillm###",args.max_length)
    ...
    def run_sample(self, num_rollouts_per_device: int = 1024, iter_count: int = 0):
        ...
        while len(ppo_rl_elements) < num_rollouts_per_device:
            ...
            # I need to understand what their masks are doing. No labels in minillm, We are using only gen_ids from teacher oor student
            ...
            with torch.no_grad():
                ...
                # Model output tokens without input(may include start token)
                #response_ids = full_ids[:, query_ids.size(1):] # remove prompt 
                #mask = (response_ids != self.trainer.tokenizer.pad_token_id)[:, query_ids.size(-1)-1:query_ids.size(-1)+response_ids.size(-1)]
                mask = (response_ids != self.trainer.tokenizer.pad_token_id)
                lens = torch.sum(mask, dim=-1)
                gen_logits = gen_out.scores # NOTE: [b, s, h_p]
                inf_mask = torch.isinf(gen_logits)
                #print("###query_ids in run_sample (sampler)###",query_ids)
                # Run sample, create KL score
                scores = self.trainer.reward_fn(query_ids, response_ids, inf_mask=inf_mask)
                t_rewards = scores["rewards"]
                #print("###t_rewards in run_sample (sampler)###",t_rewards)
                inf_mask = scores["inf_mask"]
                #print("Generate scores shape:", gen_logits.shape if isinstance(gen_logits, torch.Tensor) else "Not a tensor")
                #print("Forward logits shape:", logits.shape)
                #print("inf_mask shape:", inf_mask.shape if isinstance(inf_mask, torch.Tensor) else "Not a tensor")
                ...
                if self.args.teacher_mixed_alpha is not None:
                    ...
                    s_response_ids = s_gen_out.sequences
                    #print("###s_response_ids in run_sample (sampler)###",s_response_ids, s_response_ids.size())
                    ...
                    #print("###s_response_ids in run_sample (sampler)###",s_response_ids)
            ...
            with torch.no_grad():
                if self.args.teacher_mixed_alpha is not None:
                    ...
                    # Mixture of teacher & student distribution
