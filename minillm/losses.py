...
class Loss():
    ...
    def get_input_batch(self, ppo_batch: PPORLBatch, pt_batch=None):
        ...
        pt_input_batch, additional_pt_label = pt_batch
        pt_input_batch["labels"] = additional_pt_label["labels"]
        #print("pt_input_batch", pt_input_batch)
        #print("ppo_input_batch", ppo_input_batch)
        # merge batch
        assert len(ppo_input_batch) == len(pt_input_batch), list(ppo_input_batch.keys())
        input_batch = {}
        # If ppo_input_batch['input_ids'] has shape [32, 512]
        # and pt_input_batch['input_ids'] has shape [32, 512]
        # Then input_batch['input_ids'] will have shape [64, 512]
    ...
    def pt_loss(self, batch, logits):
        ...
        loss_mask = (no_model_batch["labels"] != -100).int()
        if self.args.model_parallel:
            lm_losses = mpu.parallel_cross_entropy(logits.contiguous().float(), no_model_batch["labels"]).view(-1)
            lm_loss = (lm_losses * loss_mask.view(-1)).sum(-1) / loss_mask.view(-1).sum(-1)
        else:
            loss_fn = nn.CrossEntropyLoss(ignore_index=-100)
            lm_loss = loss_fn(logits.view(-1, logits.size(-1)), no_model_batch["labels"].view(-1))
