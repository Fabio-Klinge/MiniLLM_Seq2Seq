...
def get_teacher_model(args, device):
    ...
    if args.model_parallel:
        ...
        model = AutoModelForSeq2SeqLM.from_pretrained(args.teacher_model_path, config=config, device_map={"": device}, torch_dtype=torch.float16)
...
def prepare_dataset(args, tokenizer):
    ...
    # Actually a PRNG: Same seed will always lead to the same sequence of numbers (deterministic)
...
def get_teacher_lm_loss(args, tokenizer, model, teacher_model, model_batch):
    ...
    # Excludes padding tokens to -100
...
def finetune(args, tokenizer: AutoTokenizer, model: deepspeed.DeepSpeedEngine, optimizer: AdamW, lr_scheduler, dataset, device, teacher_model=None):
    ...
    # print_inspect(model, '*')
    if args.model_parallel:
        dp_world_ssize = mpu.get_data_parallel_world_size()
...
def evaluate(args, tokenizer, model, dataset: LMTrainDataset, split, epoch, device):
    ...
    # Preprocessing or transformation (dimensions, padding...) of samples into batches
...
def main():
    ...
    # Load whole dataset, obtain single samples in def finetune/collate
